...
namespace Splines
{
    ...
    SplineSet::SplineSet (string const& name)
      : _name (name), baseValue (name + "_values"), basePointer (name + "_pointers"), _npts (0), _nspl (0), _X (nullptr), _Y (nullptr), _Yp (nullptr), _Ypp (nullptr), _Ymin (nullptr), _Ymax (nullptr), lastInterval (0)
    {
    }
        ...
        for (integer spl = 0; spl < nspl; ++spl)
        {
            switch (stype[size_t (spl)])
            {
                ...
                case SPLINE_VEC_TYPE:
                    //default:
                    SPLINE_ASSERT (false,
                                   "SplineSet::build\nAt spline n. " << spl << " named " << headers[spl] << " cannot be done for type = " << stype[spl]);
            }
        }
        ...
        for (size_t spl = 0; spl < size_t (nspl); ++spl)
        {
            ...
            switch (stype[size_t (spl)])
            {
                ...
                case SPLINE_VEC_TYPE:
                    ...
                    SPLINE_ASSERT (false,
                                   "SplineSet::build\nAt spline n. " << spl << " named " << headers[spl] << "\n"
                                                                     << stype[size_t (spl)] << " not allowed as spline type\nin SplineSet::build for " << spl << "-th spline");
            }
                              ...
                              real_type& x) const
    {
        ...
        SPLINE_ASSERT (zeta >= X[0] && zeta <= X[size_t (_npts - 1)],
                       "SplineSet, evaluation at zeta = " << zeta << " is out of range: [" << X[0] << ", " << X[size_t (_npts - 1)] << "]");
                              ...
                              vector<real_type>& vals) const
    {
        ...
        vals.resize (size_t (_nspl));
        for (size_t i = 0; i < size_t (_nspl); ++i)
            vals[i] = splines[i]->DDD (x) * dt3 + 3 * splines[i]->DD (x) * dt * ddt + splines[i]->D (x) * dddt;
    }
                              ...
                              integer incy) const
    {
        ...
        size_t ii = 0;
        for (size_t i = 0; i < size_t (_nspl); ++i, ii += size_t (incy))
            vals[ii] = splines[i]->DDD (x) * dt3 + 3 * splines[i]->DD (x) * dt * ddt + splines[i]->D (x) * dddt;
    }
