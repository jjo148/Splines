...
namespace Splines
{
    ...
    void
        SplineSurf::info (std::ostream& s) const
    {
    ...
    void
        SplineSurf::build (real_type const x[], integer incx, real_type const y[], integer incy, real_type const z[], integer ldZ, integer nx, integer ny, bool fortran_storage, bool transposed)
    {
        X.resize (size_t (nx));
        Y.resize (size_t (ny));
    ...
    void
        SplineSurf::build (real_type const z[], integer ldZ, integer nx, integer ny, bool fortran_storage, bool transposed)
    {
    ...
    */
        ...
        {
            build (&x.front(), 1, &y.front(), 1, &gc_z.get_mat_real().front(), integer (gc_z.get_mat_real().numRows()), nx, ny, fortran_storage, transposed);
        }
        else if (GC_VEC_REAL == gc_z.get_type())
        {
            ...
            build (&x.front(), 1, &y.front(), 1, &z.front(), ldz, nx, ny, fortran_storage, transposed);
        }
        else
        {
            SPLINE_ASSERT (false,
                           "[SplineSurf[" << _name << "]::setup] field `z` expected to be of type `mat_real_type` or  `vec_real_type` found: `" << gc_z.get_type_name() << "`");
        }
    }
#endif
